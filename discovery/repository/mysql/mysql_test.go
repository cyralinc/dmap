...
func initRepoTest(t *testing.T) (context.Context, *sql.DB, sqlmock.Sqlmock, *mySqlRepository) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	return ctx, db, mock, &mySqlRepository{
		genericSqlRepo: genericsql.NewGenericSqlRepositoryFromDB("repoName", RepoTypeMysql, "dbName", db),
	}
}
